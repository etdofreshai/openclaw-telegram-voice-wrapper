...
export interface VADOptions {
  ...
  onSpeechEnd?: () => void;
  /** Callback when speech was too short (below minSpeechMs) */
  onSpeechTooShort?: () => void;
...
export class VAD {
  ...
  constructor(options: VADOptions = {}) {
    ...
    this.onSpeechTooShort = options.onSpeechTooShort;
  ...
  private analyze(): void {
    ...
    if (rms > effectiveThreshold) {
      ...
      if (now - this.lastSpeechTime > this.silenceMs) {
        const speechDuration = this.lastSpeechTime - this.speechStartTime; // actual speech only, not silence tail
        this.isSpeaking = false;
        if (speechDuration >= this.minSpeechMs) {
          this.onSpeechEnd?.();
        } else {
          this.onSpeechTooShort?.();
        }
      }
    }
  }
